...
./control.sh start $serverlist $rdmadevice
./control.sh start $clientlist $rdmadevice
...
outfile=nsdperf-internal-${SLURM_JOB_ID}-$(date +%b-%d-%Y-%H-%M).out
